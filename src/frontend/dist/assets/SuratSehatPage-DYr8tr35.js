import { g as useGetHealthCertificates, u as useGetPatients, h as useGetDoctors, N as useCreateHealthCertificate, O as useDeleteHealthCertificate, r as reactExports, j as jsxRuntimeExports, B as Button, c as ue } from "./index-DyL3gOAQ.js";
          ...
          Button,
          {
            ...
            onClick: () => {
              const rows = (certs ?? []).map((c) => {
                const patient = patients == null ? void 0 : patients.find((p) => p.id === c.patientId);
                const doctor = doctors == null ? void 0 : doctors.find((dd) => dd.id === c.doctorId);
                return {
                  Patient: (patient == null ? void 0 : patient.name) ?? String(c.patientId),
                  Doctor: (doctor == null ? void 0 : doctor.name) ?? String(c.doctorId),
                  Purpose: c.purpose,
                  "Issued Date": new Date(
                    Number(c.issuedDate / BigInt(1e6))
                  ).toLocaleDateString(),
